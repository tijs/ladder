...
Swift library for accessing the macOS Photos library. Consumed via SPM by [attic](https://github.com/tijs/attic) (CLI) and the attic menubar app. Part of the photo-cloud system.

## Commands

```bash
swift build -c release | xcsift    # Build
swift test | xcsift                 # Run tests (58 tests)
swiftlint --fix                    # Auto-fix lint issues
```

## Architecture

LadderKit ships a single library product. Key types:
